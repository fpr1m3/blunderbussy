...
3. Query skills DB for each service → create Vector/Technique nodes
...
1. **Dame reads CAS** → Initializes PTT
2. **Dame queries skills DB** → Populates technique nodes
3. **Dame executes technique** → Updates PTT status
4. **Dame encounters error** → Classifies, logs, applies recovery
5. **Dame achieves success** → Propagates findings up tree
6. **Flag captured** → Records in loot, updates host status
7. **Session ends** → PTT persists for continuation
